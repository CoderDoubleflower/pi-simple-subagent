...
function uniqueTaskName(value: string): string {
	...
	if (!/^[a-z0-9][a-z0-9_]{0,63}$/.test(normalized)) throw new Error("task_name must contain lowercase letters, digits, and underscores, start with a letter or digit, and be at most 64 characters.");
	return normalized;
}
function nonEmptyTask(value: string): string { const task = value.trim(); if (!task) throw new Error("Subagent message must not be empty."); return task; }
function resolveModel(override: string | undefined, profile: string | undefined, root: string, parent: string | undefined): string | undefined {
	for (const value of [override, profile, root, parent]) if (typeof value === "string" && value.trim() && value.trim() !== "inherit") return value.trim();
	return undefined;
}
...
function normalizeResolvedTools(value: string[] | "none"): ResolvedToolSelection {
	...
	const tools = [...new Set(value.map((tool) => tool.trim()).filter(Boolean))]; return tools.length ? tools : "none";
}
function resolveTools(override: string[] | "none" | undefined, profile: ToolSelection | undefined, root: ToolSelection, parent: string[]): ResolvedToolSelection {
	...
	return normalizeResolvedTools(root !== "inherit" ? root : parent);
}
function abortError(): Error { const error = new Error("wait_agent was aborted."); error.name = "AbortError"; return error; }
...
export class AgentManager {
	...
	async spawn(request: SpawnAgentRequest, parent: ParentDispatchDefaults, signal?: AbortSignal): Promise<AgentSnapshot> {
		...
		this.subscriptions.set(id, agent.subscribe((snapshot) => { for (const listener of this.listeners) listener(snapshot); }));
		try { await agent.start(signal); return agent.snapshot; }
		catch (error) {
			this.agents.delete(id); this.subscriptions.get(id)?.(); this.subscriptions.delete(id);
			await agent.close().catch(() => undefined); throw error;
		}
	}
	async sendInput(target: string, message: string, interrupt: boolean, signal?: AbortSignal): Promise<{ submissionId: string; snapshot: AgentSnapshot }> {
		const agent = this.require(target); const submissionId = await agent.sendInput(nonEmptyTask(message), interrupt, signal);
		return { submissionId, snapshot: agent.snapshot };
	}
	/** Low-level bounded wait; model-facing waits use SubagentCoordinator. */
	async wait(targets: string[], timeoutMs: number | undefined, signal?: AbortSignal, onUpdate?: (snapshots: AgentSnapshot[]) => void): Promise<WaitResult> {
		...
		if (!uniqueTargets.length) throw new Error("wait_agent requires at least one non-empty target.");
		const found = uniqueTargets.map((target) => ({ target, agent: this.find(target) }));
		const live = found.flatMap((item) => item.agent ? [item.agent] : []);
		const effectiveTimeout = Math.max(0, Math.min(timeoutMs ?? this.configValue.defaultWaitTimeoutMs, this.configValue.maxWaitTimeoutMs));
		const emit = () => onUpdate?.(live.map((agent) => agent.snapshot)); emit();
		if (signal?.aborted) throw abortError();
		if (live.length && !live.some((agent) => isTerminal(agent.snapshot)) && effectiveTimeout > 0) {
			await new Promise<void>((resolve, reject) => {
				let done = false; const unsubscribers: Array<() => void> = [];
				const finish = (error?: Error) => {
					if (done) return; done = true; clearTimeout(timer); signal?.removeEventListener("abort", onAbort);
					for (const unsubscribe of unsubscribers) unsubscribe(); if (error) reject(error); else resolve();
				};
				const onAbort = () => finish(abortError()); const timer = setTimeout(() => finish(), effectiveTimeout);
				for (const agent of live) {
					const unsubscribe = agent.subscribe(() => { emit(); if (live.some((candidate) => isTerminal(candidate.snapshot))) finish(); });
					unsubscribers.push(unsubscribe); if (done) unsubscribe();
				}
	...
	async close(target: string): Promise<CloseResult> {
		const agent = this.find(target.trim()); if (!agent) return { previousSnapshot: { status: "not_found" } };
		const previousSnapshot = agent.snapshot; await agent.close(); const snapshot = agent.snapshot;
		this.subscriptions.get(snapshot.id)?.(); this.subscriptions.delete(snapshot.id); this.agents.delete(snapshot.id);
		return { previousSnapshot, snapshot };
	}
	list(): AgentSnapshot[] { return [...this.agents.values()].map((agent) => agent.snapshot).sort((a, b) => a.startedAt - b.startedAt); }
	async shutdown(): Promise<void> {
		await Promise.allSettled([...this.agents.values()].map((agent) => agent.close()));
		for (const unsubscribe of this.subscriptions.values()) unsubscribe();
		this.subscriptions.clear(); this.agents.clear(); this.listeners.clear();
	}
	private find(target: string): AgentProcess | undefined { return this.agents.get(target) ?? [...this.agents.values()].find((agent) => agent.snapshot.taskName === target); }
	private require(target: string): AgentProcess { const agent = this.find(target.trim()); if (!agent) throw new Error(`Unknown subagent: ${target}`); return agent; }
	private resolveSettings(request: SpawnAgentRequest, parent: ParentDispatchDefaults): ResolvedAgentSettings {
		const profileName = resolveProfileName(request.profileName, this.configValue); const profile = this.configValue.profiles[profileName];
		...
		const selected = resolveTools(request.tools, profile.tools, this.configValue.tools, parent.tools);
		const excluded = new Set(this.configValue.process.excludeTools);
		const tools = selected === "none" ? "none" : normalizeResolvedTools(selected.filter((tool) => !excluded.has(tool)));
		...
		const systemPrompt = [`You are subagent "${request.taskName}" working for a parent coding agent.`,
			"Complete only the delegated task. Keep the result concise, evidence-based, and directly usable by the parent agent.", profile.systemPrompt ?? ""].filter(Boolean).join("\n\n");
		return { profileName, profileDescription: profile.description, systemPrompt, model, effort, tools, cwd,
